...
public class MatchRequestController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult<MatchRequestDTO>> CreateMatchRequest([FromBody] MatchRequest matchRequest)
    {
        var matchRequests = await _MatchRequestRepo.CreateMatchRequest(matchRequest);
        var matchRequestsDtos = _mapper.Map<List<MatchRequestDTO>>(matchRequests);
        return Ok(matchRequestsDtos);
    }
